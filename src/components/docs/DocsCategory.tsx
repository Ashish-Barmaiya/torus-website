    ...
    <article className="min-w-0 px-5 py-14 sm:px-8 sm:py-20 lg:px-12">
      <div className="mx-auto max-w-[900px]">
        ...
        <header className="mt-5 border-b border-[var(--line)] pb-12">
          ...
          <p className="mt-5 max-w-2xl leading-[1.7] text-[var(--ink-soft)] text-[var(--text-body)]">
            A focused index of {group.items.length}{" "}
            {group.items.length === 1 ? "article" : "articles"} maintained as part of the Torus
          ...
          <div className="border-t border-l border-[var(--line)]">
            {group.items.map((doc, index) => (
              <Link
                key={doc.slug.join("/")}
                href={hrefForDoc(doc)}
                className="group grid gap-3 border-r border-b border-[var(--line)] px-5 py-5 transition-colors hover:bg-[var(--paper-deep)] focus-visible:relative focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--signal)] sm:grid-cols-[3rem_minmax(0,1fr)_auto] sm:items-start"
              >
                <span className="font-[family-name:var(--font-ibm-plex-mono)] text-[11px] text-[var(--ink-faint)]">
                  {String(index + 1).padStart(2, "0")}
                </span>
                <span>
                  <span className="block text-base font-semibold text-[var(--ink)] transition-colors group-hover:text-[var(--signal-dark)] dark:group-hover:text-[var(--signal)]">
                    {doc.title}
                  </span>
                  <span className="mt-1 block max-w-xl text-sm leading-6 text-[var(--ink-soft)]">
                    {doc.summary}
                  </span>
                </span>
                <span className="font-[family-name:var(--font-ibm-plex-mono)] text-[11px] text-[var(--ink-faint)]">
