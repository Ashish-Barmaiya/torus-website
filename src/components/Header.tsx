          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                className={`inline-flex h-full items-center border-b-2 font-normal tracking-[0.01em] text-[var(--text-body-compact)] transition-colors focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--signal)] ${isActive ? "border-[var(--signal)] text-[var(--ink)]" : "border-transparent text-[var(--ink-soft)] hover:border-[var(--line-strong)] hover:text-[var(--ink)]"}`}
              >
                {item.label}
              </Link>
            );
