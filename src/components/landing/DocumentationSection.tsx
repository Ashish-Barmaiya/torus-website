      ...
      <div className="grid gap-12 lg:grid-cols-[0.8fr_2fr] lg:gap-24">
        ...
        <div className="divide-y divide-[var(--line)] border-t border-[var(--line)]">
          {documentation.map(([number, title, description]) => (
            <Link href="/docs" key={number} className="group grid gap-4 py-6 sm:grid-cols-[48px_1fr_auto] sm:items-center">
              <span className="font-[family-name:var(--font-ibm-plex-mono)] text-xs text-[var(--ink-faint)]">{number}</span>
              <span>
                <span className="block text-lg font-medium tracking-[-0.02em] transition-colors group-hover:text-[var(--ink-soft)]">{title}</span>
                <span className="mt-1 block text-sm leading-6 text-[var(--ink-soft)]">{description}</span>
              </span>
              <span aria-hidden="true" className="font-[family-name:var(--font-ibm-plex-mono)] text-[var(--ink-faint)]">→</span>
            </Link>
          ))}
        </div>
      </div>
