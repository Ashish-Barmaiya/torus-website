...
function RuntimeCard({
  ...
  return (
    <motion.div
      ...
      className={`border p-5 transition-colors ${active ? "border-[var(--signal)] bg-[var(--paper)] dark:bg-[var(--ink)] dark:text-[var(--paper)]" : "border-[var(--line)]"}`}
    >
      <p className="font-[family-name:var(--font-ibm-plex-mono)] text-[11px] text-[var(--ink-faint)]">
        {title}
      </p>
      <p
        className={`mt-6 text-lg ${active ? "font-semibold text-[var(--signal-dark)] dark:text-[var(--paper)]" : "font-medium text-[var(--ink)]"}`}
      >
        {runtime}
      </p>
